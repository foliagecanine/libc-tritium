...
int _get_avail_desc() {
	for (int i = 1; i < FOPEN_MAX; i++) {
		if (!(_filedesc[i].flags & STDIO_FLAG_USED))
			return i;
	}
	return -1;
}
...
int open(const char *path, int flags, ...) {
	...
	if (flags & O_RDONLY)
		_filedesc[desc].flags |= STDIO_FLAG_READ;
	if (flags & O_WRONLY)
		_filedesc[desc].flags |= STDIO_FLAG_WRITE;
...
int read(int fd, void *buf, size_t count) {
	...
	if (_filedesc[fd].ptr > _filedesc[fd].file->size)
		return 0;
...
char *fgets(char *buf, int n, FILE *fp) {
	...
	for (i = 0; i < n-1; i++) {
		int c = fgetc(fp);
		if (c < 0)
			return NULL; // Indicators already set by fgetc
		buf[i] = (char)c;
		if (c == 0 || c == '\n')
			break;
	}
...
int vfprintf(FILE *fp, const char *format, va_list parameters) {
	_current_fp = fp;
	return _printf_template(_write_current_fp, format, parameters);
}
