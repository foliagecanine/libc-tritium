...
void perror(const char *s) {
	if ((unsigned int)errno > sizeof(_errno_strings) / sizeof(_errno_strings[0]) || _errno_strings[errno] == NULL)
...
const char *strerror(int e) {
	if ((unsigned int)e > sizeof(_errno_strings) / sizeof(_errno_strings[0]) || _errno_strings[e] == NULL)
