...
FILE _stdin = {
    ...
    .clusterNumber = 0,
    .dir_entry = 0,
    .flags = STDIO_FLAG_STDIN | STDIO_FLAG_READ,
    .desc = 0,
    .valid = 1,
    .directory = 0,
    .writelock = 0
...
FILE _stdout = {
    ...
    .clusterNumber = 0,
    .dir_entry = 0,
    .flags = STDIO_FLAG_STDOUT | STDIO_FLAG_WRITE,
    .desc = 1,
    .valid = 1,
    .directory = 0,
    .writelock = 0
...
FILE _stderr = {
    ...
    .clusterNumber = 0,
    .dir_entry = 0,
    .flags = STDIO_FLAG_STDERR | STDIO_FLAG_WRITE,
    .desc = 2,
    .valid = 1,
    .directory = 0,
    .writelock = 0
...
int read(int fd, void *buf, size_t count)
{
    ...
    if (_filedesc[fd].ptr > (long)_filedesc[fd].file->size) return 0;
    ...
    long size_remain = (long)(_filedesc[fd].file->size) - _filedesc[fd].ptr;
    if (size_remain < 0 || (size_t)size_remain < count)
    {
        if (readfile(_filedesc[fd].file, (char *)buf, _filedesc[fd].ptr, size_remain))
        {
            _set_errno(EIO);
            return -1;
        }
    ...
    if (readfile(_filedesc[fd].file, (char *)buf, _filedesc[fd].ptr, count))
    {
        _set_errno(EIO);
        return -1;
    }
...
off_t lseek(int fd, off_t offset, int whence)
{
    ...
    switch (whence)
    {
        ...
        case SEEK_SET:
            ...
            return _filedesc[fd].ptr;
        case SEEK_END:
            if ((unsigned long)(-offset) > _filedesc[fd].file->size)
            {
                _set_errno(EINVAL);
                return -1;
            }
            if (UINT64_MAX - _filedesc[fd].file->size < (unsigned long)offset)
            {
                _set_errno(EOVERFLOW);
                return -1;
            }
...
{
    return lseek(fp->desc, offset, whence) < 0 ? -1 : 0;
}
...
int fputs(const char *s, FILE *fp)
{
    ...
    int retval = write(fp->desc, s, strlen(s));

    if (retval < 0 || (size_t)retval < strlen(s))
    {
        fp->flags |= _FILE_FLAGS_ERR;
        return -1;
    }
...
size_t fread(void *ptr, size_t size, size_t nmemb, FILE *fp)
{
    ...
    if (retval < 0 || (size_t)retval < size * nmemb) fp->flags |= _FILE_FLAGS_EOF;
...
FILE *_current_fp;
int  _write_current_fp(const char *s, size_t n)
{
    return fwrite(s, 1, n, _current_fp);
}
