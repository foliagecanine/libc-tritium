 ...
 * It is not particularly efficient with small allocations, but handles large
 * ones fairly well.
 *
 * When an allocation is made, it attempts to make a new allocation in the
 * remaining unused space. If there is not enough space, it will simply allocate
 * more memory than needed.
 *
 * In some cases, a zero-length allocation will be made in the remaining space.
 * The effect is no different than if no allocation were made EXCEPT if the
 * allocation after the zero-length one is freed, it can reclaim the space used
 * by the freed allocation's alloc_t structure.
 *
 * The free function will attempt to reclaim the allocation after the freed
 * allocation, as well as before the freed allocation: [Used][Unused][To be
 * freed][Unused][Used] -> [Used][Unused][Used] This should prevent any memory
 * from being lost.
 *
 * The realloc function will try to merge the allocation with the current alloc
 * or the alloc after the current alloc (if it's free). If both of these fail,
 * it will call malloc with the new size and copy the existing data.
 *
 * The calloc function just calls the malloc function and memsets it.
 *
 * However, one of the major failures of this implementation is its ability for
 * the linked list to be corrupted by a buffer overflow.
 *
 * TODO:
 * If the last allocation passes through a 4096 byte boundary and is freed, give
...
void _init_malloc()
{
    ...
    if (!grow_heap()) exit(1); // Kill process if not enough memory to begin heap.
...
void *malloc(size_t size)
{
    ...
    while (true)
    {
        ...
        if (current_alloc->size > size)
        {
            ...
            if (new_alloc->next == NULL) last_alloc = new_alloc;
...
void free(void *ptr)
{
    ...
    if (next_alloc && !next_alloc->used)
    {
        ...
        if (current_alloc->next == NULL) last_alloc = current_alloc;
    }
    ...
    if (prev_alloc && !prev_alloc->used)
    {
        ...
        if (prev_alloc->next == NULL) last_alloc = prev_alloc;
    }
}
